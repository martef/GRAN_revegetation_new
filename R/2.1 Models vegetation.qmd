...
5.  bayes.brms5 \<- brm(Sphagnum_beta/100 \~ Treatment + t_year + (1 \| Location/Sublocation/Block), data=reveg_var, zero_inflated_beta(link = "logit", link_phi = "log", link_zi = "logit"),

6.  bayes.brms6 \<- brm(Sphagnum_beta/100 \~ Treatment \* t_year + (1 \| Location/Sublocation/Block), data=reveg_var, zero_inflated_beta(link = "logit", link_phi = "log", link_zi = "logit"), WITH ALL PLOTS
...
Model run with zero inflated beta distribution. Sphagnum cover tweeked to fit between 0 and 1 with no direct 0s and 1s, over Treatment + t_year. Without the reference plots and the plots with no water table measurements, so it's comparable to model 2.
