...
```{r}
# Create aggregated dataset to plot level
sph_long_agg <- sph_long_y3 %>%
  group_by(Location, Sublocation, Block, Treatment_ID, Sph_sp) %>%
...
sph_long_agg_ex <- sph_long_agg %>%
  filter(Sph_sp==(c('Sph ang', 'Sph cap', 'Sph pap', 'Sph ten')))
```
...
```{r}
denplot6 <- ggplot(sph_long_agg_ex, aes(x = cover, y = Sph_sp, fill = Sph_sp)) + 
  geom_density_ridges(
...
```{r}
denplot7 <- ggplot(sph_long_agg_ex, aes(x = Proportion, y = Sph_sp, fill = Sph_sp)) + 
  geom_density_ridges(
