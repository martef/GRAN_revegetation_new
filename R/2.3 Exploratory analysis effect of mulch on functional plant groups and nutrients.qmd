...
filtered_data <- reveg_var_pca_nut_temp %>%
  filter(complete.cases(across(Sphagnum:mean_soil_gs_min))) %>%
  select(-c(Mulch, Bare_peat, Litter, Dead_wood, year, Dead_sph))

# Remove columns with zero variance
non_zero_var_columns <- filtered_data %>%
  select(Sphagnum:mean_soil_gs_min) %>%
  select(where(~ var(.) != 0))

# Step 1: Log-transform only the variables where it's appropriate (optional)
...
envfit_vectors_nut_temp_s <- envfit_vectors_nut_temp_s %>%
  mutate(variables = ifelse(variables == "mean_soil_gs_min", "Min Temp", variables))
...
ggsave(nmds_CM_env_s, filename='../figures/nmds_CM_env_all.png')
```
...
```{r}
ggsave(nmds_CM_env_st, filename='../figures/nmds_CM_env_all_split.png')
```
