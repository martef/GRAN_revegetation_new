...
nmds_scores <- nmds_scores %>%
  arrange(Sublocation, Block, Treatment)
...
nmds_plot <- ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2)) +
  geom_line(data=paired_lines, aes(group = interaction(Location, Sublocation, Block)), linetype = "dashed", alpha = 0.5) +  # Connect points from the same Block within Sublocation
  geom_point(aes(color = Treatment, shape = Location), size = 3) +
...
Several pairs of C-M are not displayed with dotted lines.

When checking the paired_lines dataset, I see that I lack BSM 1-1, HM 1-1 and HM 1-4. This explains some of the lacking lines, but not all.

Where in the process do I loose them?

```{r}
count_per_block <- nmds_scores %>%
  group_by(Location, Sublocation, Block) %>%
  summarise(count = n())

print(count_per_block %>% filter(count != 2))
```

Seems like I have no nutrient data for BSM 1-1-M, and HM 1-4- C, while I have two sets nutrient data for HM 1-2-C.

When checking the original data for nutrients the error exists already there. I can't find notes about loosing/breaking any of the lacking ones, or having two from one. I know I registered one less for 2021 (BSM) when sending for analysis, which indicates the one lacking from BSM is truly missing. So it's the one in HM which is mislabeled, but which one of them?

```{r}
# View nutrient levels for HM 1-2-C, HM 1-2-M, and HM 1-4-M
target_nutrients <- filtered_data %>%
  filter(
    (Location == "HM" & Sublocation == "1" & Block == "2" & Treatment %in% c("C", "M")) |
    (Location == "HM" & Sublocation == "1" & Block == "4" & Treatment == "M")
    ) %>%
...
I'll rename this one plot and then rerun the NMDS.

```{r}
# Step 1: Prepare the Data
# Filter out rows with any NA values in the relevant columns
# Rename the plot with HM 1-2-C and NH4_N=13 to HM-1-4-C
filtered_data <- reveg_var_pca_nut %>%
  mutate(Block = ifelse(Location == 'HM' & Sublocation == '1' & Block == '2' & Treatment == 'C' & NH4_N == 13, '4', Block)) %>%
  filter(complete.cases(across(Sphagnum:Cd))) %>%
  select(-c(Mulch, Bare_peat, Litter, Dead_wood))
...
# Extract vectors for plotting (you can add them to the ggplot using geom_segment)
envfit_vectors <- as.data.frame(scores(envfit_result, display = "vectors"))
envfit_vectors$variables <- rownames(envfit_vectors)

# Add these vectors to your NMDS plot (optional)
nmds_plot <- nmds_plot +
...
```{r}
# Step 5: Plot NMDS with Corrected Legends and Connect Blocks
nmds_plot2 <- ggplot(nmds_scores2, aes(x = NMDS1, y = NMDS2)) +
  geom_point(aes(fill = Treatment, shape = Location), size = 3, alpha = 0.5, color = "black") +
  geom_line(data = paired_lines2, aes(group = interaction(Location, Sublocation, Block)), linetype = "dashed", alpha = 0.8, size = 0.5) +  # Increase line size to make overlapping lines more visible
    # Add transparency to points to distinguish overlapping points, Use fill instead of color
...
envfit_result2 <- envfit(nmds_result2, full_matrix2, permutations = 999)
env_vectors2 <- as.data.frame(scores(envfit_result2, display = "vectors"))
env_vectors2$variables <- rownames(env_vectors2)

# Create a plot for environmental vectors
env_plot2 <- ggplot(env_vectors2, aes(x = NMDS1, y = NMDS2)) +
...
envfit_vectors_scaled <- envfit_vectors2 %>% mutate(NMDS1 = NMDS1 * 2, NMDS2 = NMDS2 * 2)

nmds_plot2_evn <- nmds_plot2 +
  geom_segment(data = envfit_vectors_scaled, aes(x=0, y=0, xend = NMDS1, yend = NMDS2),
               arrow = arrow(length = unit(0.3, "cm")), color = "darkblue", alpha=0.9, size=1) +
  geom_text_repel(data = envfit_vectors, aes(x = NMDS1, y = NMDS2, label = variables), size = 4, color="black") +
  labs(
       x = "NMDS1",
       y = "NMDS2"
  ) +
...
print(nmds_plot2_evn)

```

Having both in same figure ends up a mess.

```{r}
nmds_pft_env <- grid.arrange(nmds_plot2, env_plot2, ncol = 2)

print(nmds_pft_env)
```

```{r}
#ggsave(nmds_pft_env, filename='../figures/nmds_pft_env.png')
#ggsave(nmds_plot2, filename='../figures/nmds_pft.png')
```
...
env_vectors3 <- as.data.frame(scores(envfit_result3, display = "vectors"))
env_vectors3$variables <- rownames(env_vectors3)

# Create a plot for environmental vectors
# Set the same xy-limits as the NMDS plot
define_limits_x <- c(min(nmds_scores3$NMDS1, env_vectors3$NMDS1), max(nmds_scores3$NMDS1, env_vectors3$NMDS1))
define_limits_y <- c(min(nmds_scores3$NMDS2, env_vectors3$NMDS2), max(nmds_scores3$NMDS2, env_vectors3$NMDS2))

env_plot4 <- ggplot(env_vectors3, aes(x = NMDS1, y = NMDS2)) +
  coord_cartesian(xlim = define_limits_x, ylim = define_limits_y) +
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
               arrow = arrow(length = unit(0.3, "cm")), color = "blue", alpha = 0.9, size = 1) +
  geom_text_repel(aes(x = NMDS1, y = NMDS2, label = variables), size = 3) +
  labs(
       x = "NMDS1",
       y = "NMDS2"
...
# Create a plot for environmental vectors
env_plot3 <- ggplot(env_vectors3, aes(x = NMDS1, y = NMDS2)) +
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
               arrow = arrow(length = unit(0.2, "cm")), color = "darkblue", alpha = 0.7) +
  geom_text_repel(aes(x = NMDS1, y = NMDS2, label = variables), size = 3) +
  labs(
...
env_plot3b <- geom_segment(data = envfit_vectors_scaled, aes(x=0, y=0, xend = NMDS1, yend = NMDS2),
               arrow = arrow(length = unit(0.3, "cm")), color = "darkblue", alpha=0.9, size=1) +
  geom_text_repel(data = envfit_vectors, aes(x = NMDS1, y = NMDS2, label = variables), size = 4, color="black") +
  labs(
       x = "NMDS1",
...
```{r}
nmds_pft_env2 <- grid.arrange(nmds_plot3, env_plot3, ncol = 2)

print(nmds_pft_env2)
```

```{r}
nmds_pft_env3 <- grid.arrange(nmds_plot3, env_plot4, ncol = 2)
