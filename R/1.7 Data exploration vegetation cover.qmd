...
```{r}
reveg_var%>%
  dplyr::select(c(Location, t_year, Sublocation, Block, Treatment, Treatment_ID, Sphagnum, Other_mosses, Liverworts, Lichen, Polytrichum, Racomitrium, Trichophorum, Eriopherum, Evergreens, Narthecium, Other_veg, Veg_Ground, Tot_Ground, Field, Total_veg))%>%
...
mean_cover_ref <- reveg_var %>%
  ...
  summarise(
    ...
    mean_Eriopherum = mean(Eriopherum, na.rm = TRUE),
    max_Eriopherum = max(Eriopherum, na.rm = TRUE),
    ...
    mean_Evergreens = mean(Evergreens, na.rm = TRUE),
    max_Evergreens = max(Evergreens, na.rm = TRUE),
    mean_Litter = mean(Litter, na.rm = TRUE),
    max_Litter = max(Litter, na.rm = TRUE),
    mean_OtherMosses = mean(Other_mosses, na.rm = TRUE),
    max_OtherMosses = max(Other_mosses, na.rm = TRUE)
  ) %>%
  pivot_longer(
    cols = everything(),
...
The reference plots have a high cover of Sphagnum (about 71% mean), and a decent cover of Narthecium, evergreens (mostly heather) and Trichophorum. There's also some litter (about 14%ish), while Eriopherum covers only ca. 3%)
...
There is however a number here I need to check further. How can there be max 80% cover of Eriopherum in M in year 2, while this is only 30% in year 3? That is a drastic decrease. I have narrowed the problem down to VSM2:

```{r}
reveg_var %>%
  filter(Treatment=='M' & Station_ID=='VSM-2') %>%
  select(c(Location, Sublocation, Block, t_year, Eriopherum, Mulch, Bare_peat, Sphagnum, Polytrichum))
```
...
```{r}
plot_hea <- reveg_var %>%
  filter(!is.na(Evergreens)) %>%  # Exclude the NA value
  ggplot() +   
  geom_boxplot()+   
  aes(x=Treatment, y=Evergreens, color=as.factor(t_year)) +
  ylab("Heathers cover") +
  labs(color = "Growth seasons since treatment") +
  theme_bw()
...
# Rearrange Treatment levels
  ggplot(aes(x = Treatment, y = Sphagnum, color = as.factor(t_year), fill=as.factor(t_year), alpha=0.8)) +   
...
ggsave(plot_SP, filename='../figures/violinplot_SP.png')
```
...
plot_OV <- reveg_var %>%
  filter(!is.na(Other_veg)) %>%  # Exclude NA values in the Sphagnum column
   mutate(Treatment = factor(Treatment, levels = c("C", "M", "S", "R")),
          Treatment = fct_recode(Treatment, "M&S" = "S")) %>% # Rearrange Treatment levels
  ggplot(aes(x = Treatment, y = Other_veg, color = as.factor(t_year), fill=as.factor(t_year), alpha=0.8)) +   
  geom_violin(scale = "width", width= 0.8) +   
  geom_point(pch = 21, size=1, position = position_jitterdodge())+
...
ggsave(plot_OV, filename='../figures/violinplot_OV.png')
```
