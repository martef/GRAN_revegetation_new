...
daily_means <- wt_season %>%
  ...
  group_by(Station_ID, Location, jday, month_name) %>%
...
#join climate and WT
clim_wt <- daily_means %>%
  left_join(climdat_season, by=c('clim_station', 'Date'))

clim_wt_combined <- clim_wt %>%
  group_by(Station_ID, Location, clim_station, Date, jday.x, month_name.x, year, Year, month, Month) %>%
  summarize(
