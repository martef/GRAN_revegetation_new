...
reveg <- reveg %>%   
  mutate(Sphagnum_nozero=case_when(Sphagnum==0~ +0.0001,                             TRUE~Sphagnum)) 
...
```{r}
reveg %>%
  select(c(Location, Sublocation, Block, Treatment, t_year, Sphagnum, Sphagnum_nozero, Sphagnum_no1, Sphagnum_beta))
```
...
climvar <- readr::read_delim('../data/clim_var.csv', 
                                  delim = ',',
                                  col_names = TRUE)
...
```{r, output=FALSE, warning=FALSE}
wtvar <- readr::read_delim('../data/WT_variables.csv', 
                                  delim = ',',
                                  col_names = TRUE)
