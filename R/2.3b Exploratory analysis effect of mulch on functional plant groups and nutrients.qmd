---
title: "2.3b Exploratory analysis effect of mulch on functional vegetation groups, nutrients and temperature"
author: "MarteF"
format: html
editor: visual
---
...
#Subsetting year 3 to only include C and M
...
#Join the two datasets of subset year 3 C & M and nutrients year 0
...
```{r}
#Join the datasets of PFTs, nutrients and temps
temp <- temp %>%
...
Is there a significant difference in overall max and min temp over treatment?
...
temp_max_brm <- readRDS('RDS-files/temp_max.RDS')
```
...
temp_min_brm <- readRDS('RDS-files/temp_min.RDS')
```
...
There is no significant effect of either maximum temp or minimum temp.

Will test max/min fluctuation later.

# NMDS Ordination

Note to self: Unlike other ordination techniques that rely on (primarily Euclidean) distances, such as Principal Coordinates Analysis, NMDS uses rank orders.
...
I now have lines connecting the various sample pairs (within same block).
...
This is too messy to have in one figure.
...
```{r}
#Create combined plot with only significant vectors
...
I don't think this is a better display than the previous.
